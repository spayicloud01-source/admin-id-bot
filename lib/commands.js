export const COMMANDS = [
  { prefix: "อ่านบัตรล่าสุด", action: "getRecentIdentityImages", permission: "แก้ข้อมูลลูกค้า", requiresQuery: false },
  { prefix: "กรอกชื่อเอง", action: "verifyCustomerIdentity", permission: "แก้ข้อมูลลูกค้า" },
  { prefix: "บัตรประชาชน", action: "verifyCustomerIdentity", permission: "แก้ข้อมูลลูกค้า" },
...
export function parseCommand(text) {
  ...
  for (const cmd of COMMANDS) {
    if (input === cmd.prefix || input.startsWith(cmd.prefix + " ")) {
      ...
      if (cmd.action === "verifyCustomerIdentity") {
        const m = rest.match(/^(\S+)\s+(\S+)\s+(\S+)(?:\s+(\d{4}))?$/);
        const firstToken = rest.match(/^(\S+)/);
        return {
          ...cmd,
          query: m ? m[1] : (firstToken ? firstToken[1] : ""),
          firstName: m ? m[2] : "",
          lastName: m ? m[3] : "",
          idLast4: m && m[4] ? m[4] : "",
        };
